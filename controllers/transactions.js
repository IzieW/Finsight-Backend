...
transactionRouter.get("/", async (request, response) => {
  const expenses = await Transaction.find({}).populate("user", {
    username: 1,
  });
...
transactionRouter.delete("/:id", async (request, response) => {
  ...
  if (
    !request.user ||
    transactionToDelete.user.toString() !== request.user.id.toString()
  ) {
  ...
  const newBalance =
    Math.round((user.balance + transactionToDelete.amount * -1) * 100) / 100;
  ...
  await User.findByIdAndUpdate(user.id, {
    ...
    transactions: user.transactions.filter(
      (n) => n.toString() !== request.params.id.toString()
    ),
  });
